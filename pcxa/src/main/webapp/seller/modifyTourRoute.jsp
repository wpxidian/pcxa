...
<html>
  ...
  <body>
    <div class="content">
       <form id="addRouteForm" action="modifyTourRoute" method="post">
        <fieldset>
          <legend>添加精品线路</legend>
          <br />
          <label id="name" for="name">线路名称</label><div id="nameTip"></div>
          <input id="name" type="text" name="name" value="${tourRoute.name}"/>
          <br /><br />
          <label for="brief_introduction">线路简介</label><div id="brief_introductionTip"></div>
          <textarea name="brief_introduction" id="brief_introduction" rows="4" cols="25">${tourRoute.briefIntroduction}</textarea>
          <br />
          <label for="introduction">线路介绍</label><div id="introductionTip"></div>
          <textarea id="introduction" name="introduction" cols="100" rows="8" style="width:700px;height:200px;visibility:hidden;">${tourRoute.introduction}</textarea>
          <input type="hidden" name="id" value="${tourRoute.tourRouteId}"/><br />
          <label for="uploadify">精品线路主图片</label>
          <div class="uploadifyTip"></div><input id="uploadify" name="uploadify" type="file"/>
          <input type="hidden" class="picture_link" name="picture_link" value="${tourRoute.pictureLink}"/>
    ...
    <script type="text/javascript">
         $(document).ready(function(){
          ...
          <%
          	 URLHelper.addUploadButton(out, "#uploadify", "旅行线路主图片", "#addRouteForm", "picture_link", false, URLHelper.PICTURE_TYPE);
