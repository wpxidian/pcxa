...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  <body style="overflow: scroll;">
    <div class="content"  style="overflow: scroll;">
       <form id="SellerAddHotelForm" action="insertHotel" method="post">
        <fieldset>
          <legend>添加酒店住宿信息</legend>
          <br /><label  for="name">酒店名称：</label><input id="name" type="text" name="name"/>
          <div class="validatorTipContainer"><div id="nameTip"></div></div>           
          <label for="phone">联系电话：</label><input id="phone" type="text" name="phone"/>
          <div class="validatorTipContainer"><div id="phoneTip"></div></div>
          <label for="link">相关链接：</label><input id="link" type="text" name="link" value="http://"/>
          <div class="validatorTipContainer"><div id="linkTip"></div></div>
          <label for="star">酒店星级：</label>
             <s:select id="star" list="#{'1':'一星级','2':'二星级','3':'三星级','4':'四星级','5':'五星级'}" name="star">				
			 </s:select>
    ...
    <script type="text/javascript">
		$(document).ready(function(){
			$.formValidator.initConfig({formID:"SellerAddHotelForm",theme:'Default',debug:false,onSuccess:function(){
			...
			<%
			  URLHelper.addUploadButton(out, "#uploadify", "酒店主图片", "#SellerAddHotelForm", "picture_link",false,URLHelper.PICTURE_TYPE);
